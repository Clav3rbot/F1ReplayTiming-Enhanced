...
def _get_driver_positions_by_time_sync(
    ...
    # Offset for timing the sector indicators. It must match the offset laps.json
    # uses for lap completion (min_date - t0_date) so sector markers land exactly
    # when the lap completes. In qualifying, session_time_offset is later
    # overwritten with the Q1 green-flag time (for the phase countdown), which
    # would make sectors trail lap completion by a few seconds — so capture the
    ...
    for i in range(_actual_frames):
        ...
        if is_race:
            ...
            if session_type in ("Q", "SQ", "FP1", "FP2", "FP3"):
                # Time sector markers with the lap-completion offset (not the quali
                # countdown's session_time_offset) so they appear exactly when the
                # lap completes, in sync with the last-lap time.
