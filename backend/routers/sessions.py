...
async def delete_session_data(year: int, round_num: int, type: str = Query("R")):
    ...
    passphrase-protected or explicitly opted in with ALLOW_REPROCESS.
    """
    if not _reprocess_allowed():
        raise HTTPException(
            status_code=403,
            detail="Deleting session data is disabled on this instance. Enable AUTH_ENABLED or set ALLOW_REPROCESS=true.",
        )
...
async def update_retention(
    ...
    if not _reprocess_allowed():
        raise HTTPException(
            status_code=403,
            detail="Retention settings are disabled on this instance. Enable AUTH_ENABLED or set ALLOW_REPROCESS=true.",
        )
    try:
        if dry_run:
